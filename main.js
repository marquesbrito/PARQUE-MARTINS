...
// CONFIGURAÇÃO DAS SENHAS (Usando o mesmo padrão de nome para consistência)
...
// Estrutura para armazenar senhas ocupadas: chave = "categoria|dia|senha" (ex: "ASPIRANTE|Sexta|001")
        ...
        try {
            ...
            snapshot.forEach((doc) => {
                ...
                if (data.categoria && data.senha && data.dia) {
                    // Usamos o nome da categoria como está salvo (ex: "ASPIRANTE")
                    const chave = `${data.categoria}|${data.dia}|${data.senha}`;
...
function isSenhaOcupada(categoriaKey, dia, senha) {
    // Pega o nome correto da categoria (ex: "ASPIRANTE")
    const categoriaNome = CATEGORIAS_CONFIG[categoriaKey]?.nome || categoriaKey;
    const chave = `${categoriaNome}|${dia}|${senha}`;
...
function getSenhasDisponiveis(categoriaKey, dia) {
    const config = CATEGORIAS_CONFIG[categoriaKey];
    if (!config) return [];
    
    // Se não tem a faixa para o dia, tenta Sexta
    ...
    for (let i = inicio; i <= fim; i++) {
        ...
        if (!isSenhaOcupada(categoriaKey, dia, numero)) {
...
function atualizarSelectSenha() {
    ...
    console.log(`🔄 Atualizando select - Categoria: ${categoriaKey}, Dia: ${dia}`);
    ...
    if (categoriaKey === "Master" || categoriaKey === "FemininoJovem") {
        dia = "Sexta";
        console.log(`📌 Forçando dia para ${dia} (categoria ${categoriaKey})`);
    }
    ...
    const disponiveis = getSenhasDisponiveis(categoriaKey, dia);
    console.log(`📋 Senhas disponíveis para ${categoriaKey} - ${dia}: ${disponiveis.length}`);
    ...
    atualizarContador(categoriaKey, dia, disponiveis.length);
    atualizarValorTotal();
}

function atualizarContador(categoriaKey, dia, total) {
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    document.getElementById("enviar").addEventListener("click", async function() {
        ...
        // Força dia para Master e FemininoJovem
        if (categoriaKey === "Master" || categoriaKey === "FemininoJovem") {
            dia = "Sexta";
        } else {
        ...
        if (isSenhaOcupada(categoriaKey, dia, senha)) {
            alert("❌ Esta senha já foi usada! Recarregue a página e tente novamente.");
            await carregarSenhasOcupadas(); // Recarrega do Firebase
            atualizarSelectSenha();
            return;
        }
        ...
        if (categoriaKey === "Aspirante" || categoriaKey === "Profissional") {
        ...
        // 🔒 Marca senha como ocupada LOCALMENTE (usando o nome correto da categoria)
        const chaveOcupada = `${categoriaNome}|${dia}|${senha}`;
        ...
        if (db) {
            try {
                await addDoc(collection(db, "senhas_registradas_2026"), {
                    ...
                    categoria: categoriaNome,  // Salva como "ASPIRANTE", "PROFISSIONAL", etc.
                    dia: dia,
                    tipoInscricao: tipo,
                    senha: senha,
        ...
        // Atualiza o select (a senha escolhida deve desaparecer)
        atualizarSelectSenha();
        limparFormulario();
